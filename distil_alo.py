...
def train():
    ...
    student_model = resnet152(weights=ResNet152_Weights.IMAGENET1K_V2)
    ...
    ### Config
    num_epochs = 50 # initial num_epochs
    phases = 10
    phase_inc = 10
    train_dir = "./Train"
    val_dir = "./Val"
    ...
    for phase in range(1, phases + 1):
        ...
        additional_layers = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(256, train_dset.total_classes)  # Adjust the output size to match the number of classes
        )
        
        if prev_weights != None:
            model[1][3].weight.data[:train_dset.total_classes-phase_inc, :] = prev_weights
            model[1][3].bias.data[:train_dset.total_classes-phase_inc] = prev_bias
        ...
        model = nn.Sequential(student_model, additional_layers)
        model.to("cuda")
        ...
        eval_validation(model, val_dir, phase)
        eval_validation(model, test_dir, phase)
        ...
        prev_weights = model[1][3].weight
        prev_bias = model[1][3].bias
